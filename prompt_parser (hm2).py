...
top_level_sequence: prompt ("::" | ":::") prompt ("::" prompt)* "!"?  // Handle ::: and ::
...
def get_learned_conditioning_prompt_schedules(prompts, base_steps, hires_steps=None, use_old_scheduling=False):
    ...
    def collect_steps(steps, tree):
        ...
        def resolve_tree(tree):
            if isinstance(tree, lark.Tree):
                children = [resolve_tree(child) for child in tree.children if not isinstance(child, str) or child.strip() not in ("!", "::", ":::")]
        ...
        class CollectSteps(lark.Visitor):
            ...
            def compound(self, tree):
                parts = [resolve_tree(child) for child in tree.children]
                res.append((steps, "_".join(parts)))

            def top_level_sequence(self, tree):
                parts = [resolve_tree(child) for child in tree.children if not isinstance(child, str) or child.strip() not in ("!", "::", ":::")]
            ...
            def nested_sequence(self, tree):
                sequence_elements = [resolve_tree(child) for child in tree.children if not isinstance(child, str) or child.strip() not in ("!", "::", ":::")]
                return f"[{' | '.join(sequence_elements)}]"

            def grouped(self, tree):
                group_descriptions = []
                for child in tree.children:
                    if isinstance(child, lark.Tree) and child.data == "alternate":
                ...
                res.append((steps, ", ".join(filter(None, group_descriptions))))
            ...
            def scheduled(self, tree):
                ...
                for child in tree.children[:-1]:
                    if isinstance(child, lark.Tree):
                        ...
                        prompts.append(str(child).strip())
                ...
                try:
                    ...
                    if v < 0:
                        ...
                        res.append((steps, prompts[0] if prompts else ""))
                        return
                    ...
                    res.append((steps, (context[-1] + " " + "".join(prompts)).strip() if context else "".join(prompts)))
                    return
                ...
                if num_prompts == 1:
                    ...
                    if context:
                        res.append((step, context[-1]))
                    res.append((steps, (context[-1] + " " + prompts[0]).strip() if context else prompts[0]))
                else:
                    step_size = (v * steps) / num_prompts
                    for i in range(num_prompts):
                        end = int((i + 1) * step_size)
                        res.append((end, (context[-1] + " " + prompts[i]).strip() if context else prompts[i]))

                if additional_info and isinstance(additional_info, str) and additional_info.lower() in ("reverse", "r"):
                    res[-num_prompts:] = res[-num_prompts:][::-1]
    ...
    def at_step(step, tree):
        class AtStep(lark.Transformer):
            def and_rule(self, args):
                return " and ".join(self._resolve_tree(arg) if isinstance(arg, lark.Tree) else str(arg) for arg in args)

            def compound(self, args):
                return "_".join(str(arg) for arg in args)

            def top_level_sequence(self, args):
                parts = [self._resolve_tree(child) if isinstance(child, lark.Tree) else str(child) 
                         for child in args if not isinstance(child, str) or child.strip() not in ("!", "::", ":::")]
                return ": ".join(parts)
            ...
            def nested_sequence(self, args):
                sequence_elements = [self._resolve_tree(desc) if isinstance(desc, lark.Tree) else desc.strip() 
                                    for desc in args if not isinstance(desc, str) or desc.strip() not in ("!", "::", ":::")]
                return f"[{' | '.join(sequence_elements)}]"
            ...
            def scheduled(self, args):
                ...
                try:
                    ...
                    return (context[-1] + " " + "".join(prompts)).strip() if context else "".join(prompts)

                num_prompts = len(prompts)
                step_size = (weight * steps) / num_prompts if num_prompts > 1 else weight
                step_intervals = []
                if num_prompts == 1:
                    end = min(int(weight) if weight >= 1 else int(weight * steps), steps)
                    step_intervals = [(0, end)]
                else:
                    step_intervals = [
                        (int(i * step_size), int((i + 1) * step_size))
                        for i in range(num_prompts)
                    ]
                ...
                for i, (start, end) in enumerate(step_intervals):
                    if start <= step <= end:
                        return (context[-1] + " " + prompts[i]).strip() if context else prompts[i]
                return context[-1] if context else prompts[0] if prompts else ""
            ...
            def grouped(self, args):
                ...
                return ", ".join(filter(None, group_descriptions))

            def start(self, args):
                return ''.join(self._resolve_tree(arg) if isinstance(arg, lark.Tree) else str(arg) for arg in args)
            ...
            def _resolve_tree(self, tree):
                if isinstance(tree, lark.Tree):
                    return "".join(self._resolve_tree(child) for child in tree.children if not isinstance(child, str) or child.strip() not in ("!", "::", ":::"))
                return str(tree).strip()
    ...
    def get_schedule(prompt):
        ...
        for t in collect_steps(steps, tree):
            prompt_at_step = at_step(t, tree).strip()
...
def parse_prompt_attention(text):
    ...
    current = ""
    tokens = list(re_attention.finditer(text))
    idx = 0
    while idx < len(tokens):
        ...
        if text_segment.startswith('\\'):
            current += text_segment[1:]
        elif text_segment == '(':
            if current:
                res.append([current, 1.0])
                current = ""
    ...
    # Сцепляем одинаковые веса (сохраняя пробелы!)
    i = 0
    while i + 1 < len(res):
