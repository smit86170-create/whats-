...
# Grammar updated to handle commas in plain text
...
def validate_prompt(prompt):
    ...
    if stack:
        raise ValueError(f"Unbalanced brackets in prompt: {prompt}. Ensure all '[' and '(' have matching ']' and ')'.")
    if '::' in prompt and not (prompt.endswith('!') or prompt.endswith('!!') or prompt.endswith('~') or prompt.endswith(';')):
...
def resolve_tree(tree):
    ...
    if isinstance(tree, lark.Tree):
        return "".join(resolve_tree(child) for child in tree.children if not isinstance(child, lark.Token) or child.type != "WHITESPACE")
    return str(tree)
...
def get_learned_conditioning_prompt_schedules(prompts, base_steps, hires_steps=None, use_old_scheduling=False, seed=None, verbose=False, telemetry=False):
    ...
    Traceback (most recent call last):
        ...
    ValueError: Unbalanced brackets in prompt: a [unbalanced. Ensure all '[' and '(' have matching ']' and ')'.
    >>> g("a [b:.5] c")
    [[5, 'a  c'], [10, 'a b c']]
    >>> g("[fe|]male")
    ...
    def collect_steps(steps, tree, prefix="", suffix="", depth=0):
        """Recursively process the parse tree to build a list of [step, prompt] schedules."""
        indent = "  " * depth
        if verbose:
            print(f"{indent}Processing node: {tree.data if isinstance(tree, lark.Tree) else tree}")

        # Base case: string token
        if isinstance(tree, str):
            if verbose:
                print(f"{indent}String: {tree}")
        ...
        if isinstance(tree, lark.Tree) and tree.data == "plain":
            ...
            if verbose:
                print(f"{indent}Plain text: {text}")
        ...
        if isinstance(tree, lark.Tree) and tree.data == "scheduled" and use_scheduling:
            if verbose:
                print(f"{indent}Found scheduled node: {tree}")
            ...
            if verbose:
                print(f"{indent}Scheduled prompts: {[resolve_tree(p) for p in prompts]}")
                print(f"{indent}Step boundary: {boundary}")
                print(f"{indent}Local schedules: {schedules}")
            return schedules

        # Alternate node: [prompt1|prompt2|...]
        if isinstance(tree, lark.Tree) and tree.data in ("alternate", "alternate1", "alternate2"):
            if verbose:
                print(f"{indent}Found alternate node: {tree}")
            options = []
            ...
            if not options:
                if verbose:
                    print(f"{indent}No valid alternate options, skipping.")
            ...
            if verbose:
                print(f"{indent}Alternate options: {options}")
                print(f"{indent}Local schedules: {schedules}")
            return schedules
        ...
        if isinstance(tree, lark.Tree) and tree.data == "grouped":
            if verbose:
                print(f"{indent}Found grouped node: {tree}")
            options = [resolve_tree(child).strip() for child in tree.children if resolve_tree(child).strip()]
            text = ", ".join(options)
            if verbose:
                print(f"{indent}Grouped text: {text}")
            return [[steps, prefix + text + suffix]]

        # Sequence node
        if isinstance(tree, lark.Tree) and tree.data == "sequence":
            if verbose:
                print(f"{indent}Found sequence node: {tree}")
            described_object = resolve_tree(tree.children[0]).strip()
            descriptors = [resolve_tree(child).strip(" ;~!") for child in tree.children[1:] if resolve_tree(child).strip()]
            text = f"{described_object}: {', '.join(descriptors)}"
            if verbose:
                print(f"{indent}Sequence text: {text}")
            return [[steps, prefix + text + suffix]]

        # Top-level sequence node
        if isinstance(tree, lark.Tree) and tree.data == "top_level_sequence":
            if verbose:
                print(f"{indent}Found top-level sequence node: {tree}")
            owner = resolve_tree(tree.children[0]).strip()
            sequences = []
            for child in tree.children[1:]:
                if isinstance(child, lark.Tree) and child.data == "sequence":
                    described_object = resolve_tree(child.children[0]).strip()
                    descriptors = [resolve_tree(c).strip(" ;~!") for c in child.children[1:] if resolve_tree(c).strip()]
                    sequences.append(f"{described_object}: {', '.join(descriptors)}")
            text = f"{owner} -> {', '.join(sequences)}"
            if verbose:
                print(f"{indent}Top-level sequence text: {text}")
            return [[steps, prefix + text + suffix]]

        # Other nodes (prompt, nested_sequence, etc.)
        if isinstance(tree, lark.Tree):
            if verbose:
                print(f"{indent}Processing children of {tree.data}")
            schedules = []
            ...
            if verbose:
                print(f"{indent}Schedules for {tree.data}: {schedules}")
            return schedules

        # Fallback for unhandled cases
        if verbose:
            print(f"{indent}Fallback: {resolve_tree(tree)}")
    ...
    def get_schedule(prompt):
        ...
        try:
            ...
            seen = set()
            unique_schedules = []
            for step, prompt_text in sorted(schedule, key=lambda x: x[0]):
                prompt_text = re.sub(r'\s+', ' ', prompt_text.strip())
                if (step, prompt_text) not in seen:
                    unique_schedules.append([step, prompt_text])
                    seen.add((step, prompt_text))
            if verbose:
                print(f"Final schedules: {unique_schedules}")
            if telemetry:
                logging.info(f"Prompt: {prompt}, Schedule: {unique_schedules}")
            ...
            raise ValueError(f"Unbalanced brackets in prompt: {prompt}. Ensure all '[' and '(' have matching ']' and ')'.") from e
