...
def validate_prompt(prompt: str) -> bool:
    """
    Validate prompt syntax for critical errors and potential issues.
    
    Args:
        prompt: The input prompt string to validate.
    
    Returns:
        bool: True if valid, False otherwise.
    
    Raises:
        ValueError: For critical syntax errors (e.g., unbalanced brackets, invalid weights).
    """
    ...
    for i, char in enumerate(prompt):
        if char in '([':
            ...
            if not stack or stack[-1][0] != '(':
                raise ValueError(f"Unbalanced parenthesis at position {i}: {prompt}")
            stack.pop()
        elif char == ']':
            if not stack or stack[-1][0] != '[':
                raise ValueError(f"Unbalanced bracket at position {i}: {prompt}")
            stack.pop()
    if stack:
        char, pos = stack[-1]
        raise ValueError(f"Unclosed {char} at position {pos}: {prompt}")
    
    # Check for unterminated sequences
    if '::' in prompt and not any(prompt.endswith(s) for s in ('!', ';', '!!')) and ':::' not in prompt:
        raise ValueError(f"Unterminated sequence in prompt: {prompt}")
    
    # Validate weights in scheduled prompts
    scheduled_matches = re.findall(r'\[([^]]*):(\d*\.?\d*)\]', prompt)
    for match in scheduled_matches:
        try:
            weight = float(match[1])
            if weight < 0:
                raise ValueError(f"Negative weight '{match[1]}' in scheduled prompt: {prompt}")
        except ValueError:
            raise ValueError(f"Invalid weight '{match[1]}' in scheduled prompt: {prompt}")
    
    # Validate weights in emphasized prompts
    emphasized_matches = re.findall(r'\(([^)]*):([-+]?\d*\.?\d+)\)', prompt)
    for match in emphasized_matches:
        try:
            weight = float(match[1])
            if weight < 0:
                raise ValueError(f"Negative weight '{match[1]}' in emphasized prompt: {prompt}")
        except ValueError:
            raise ValueError(f"Invalid weight '{match[1]}' in emphasized prompt: {prompt}")
    
    # Check for invalid characters
    if re.search(r'[\x00-\x1F\x7F]', prompt):
        raise ValueError(f"Prompt contains invalid control characters: {prompt}")
    
    # Validate AND-separated subprompts
    if " AND " in prompt:
        ...
        for subprompt in subprompts:
            try:
                ...
                raise ValueError(f"Invalid subprompt '{subprompt}' in AND-separated prompt: {str(e)}")
    
    # Check for malformed alternates
    if re.search(r'\[\s*\|\s*\]', prompt):
        raise ValueError(f"Empty alternate group found in prompt: {prompt}")
...
def resolve_tree(tree: lark.Tree | lark.Token, keep_spacing: bool = False) -> str:
    ...
    if isinstance(tree, lark.Tree):
        children = [
            resolve_tree(child, keep_spacing) for child in tree.children
            if not (isinstance(child, lark.Token) and child.type in ("WHITESPACE", "NUMBER", "COLON"))
        ]
        result = "".join(str(c) for c in children if c)
        return re.sub(r"\s+", " ", result).strip() if keep_spacing else re.sub(r"\s+", "", result).strip()
    return str(tree).strip() if keep_spacing else re.sub(r"\s+", "", str(tree)).strip()
...
def _process_tree(tree: lark.Tree, steps: int, prefix: str, suffix: str, depth: int, verbose: bool, use_scheduling: bool, seed: int | None) -> list[list[int | str]]:
    ...
    return schedules or [[steps, prefix + resolve_tree(tree) + suffix]]
...
def _process_scheduled(tree: lark.Tree, steps: int, prefix: str, suffix: str, depth: int, verbose: bool, use_scheduling: bool, seed: int | None) -> list[list[int | str]]:
    ...
    if step_range_list:
        ...
        right1 = resolve_tree(prompts[0]).strip()
        right2 = resolve_tree(prompts[1]).strip()
        schedules.append([boundary, prefix + right1 + suffix])
        schedules.append([steps, prefix + right1 + right2 + suffix])
    else:
        right = resolve_tree(prompts[0]).strip()
        schedules.append([boundary, prefix + right + suffix])
        child_schedules = collect_steps(steps, prompt, prefix, suffix, depth + 1, verbose, use_scheduling, seed)
        for step, text in child_schedules:
            if step > boundary:
...
def _process_numbered(tree: lark.Tree, steps: int, prefix: str, suffix: str, seed: int | None) -> list[list[int | str]]:
    ...
    if isinstance(target, lark.Tree) and target.data in ("alternate1", "alternate2"):
        options = [resolve_tree(c) for c in target.children if resolve_tree(c)]
    else:
...
def get_learned_conditioning_prompt_schedules(prompts: list[str], base_steps: int, hires_steps: int | None = None, use_old_scheduling: bool = False, seed: int | None = 42, verbose: bool = False, telemetry: bool = False, log_level: str = 'WARNING', strict: bool = False) -> list[list[list[int | str | PromptParsingFailed]]]:
    """
    Converts a list of prompts into schedules for Stable Diffusion.

    Supports complex scheduling, alternates, grouping, sequences, and multi-conditioning (e.g., 'a AND b'). Invalid prompts
    are returned as-is with detailed logging unless strict=True, in which case an exception is raised.

    Args:
        prompts: List of prompt strings to process.
        base_steps: Base number of steps for scheduling.
        hires_steps: Optional high-resolution steps; overrides base_steps if provided.
        use_old_scheduling: If True, use older scheduling logic.
        seed: Random seed for reproducibility, defaults to 42.
        verbose: If True, log detailed parsing information.
        telemetry: If True, log telemetry data for debugging.
        log_level: Logging level (e.g., 'DEBUG', 'INFO', 'WARNING').
        strict: If True, raise an exception for invalid prompts instead of returning as-is.

    Returns:
        list: List of schedules, where each schedule is a list of [step, text] pairs.
              Invalid prompts are wrapped in PromptParsingFailed for clarity.

    >>> get_learned_conditioning_prompt_schedules(["a [b:c:0.5] AND d"], 10, seed=42)
    [[[5, 'a b'], [10, 'a c'], [10, 'd']]]
    >>> get_learned_conditioning_prompt_schedules(["a::b"], 10, seed=42)  # doctest: +ELLIPSIS
    [[10, <prompt_parser.PromptParsingFailed object at ...>]]
    >>> get_learned_conditioning_prompt_schedules(["a [b:c:invalid]"], 10, seed=42)  # doctest: +ELLIPSIS
    [[10, <prompt_parser.PromptParsingFailed object at ...>]]
    >>> get_learned_conditioning_prompt_schedules(["[a||b]"], 10, seed=42)  # doctest: +ELLIPSIS
    [[10, <prompt_parser.PromptParsingFailed object at ...>]]
    >>> get_learned_conditioning_prompt_schedules(["a [b:c:0.5] AND d"], 10, seed=42, strict=True)
    [[[5, 'a b'], [10, 'a c'], [10, 'd']]]
    >>> try: get_learned_conditioning_prompt_schedules(["a::b"], 10, seed=42, strict=True); assert False
    ... except ValueError as e: assert "Unterminated sequence" in str(e)
    """
    ...
    @lru_cache(maxsize=CACHE_SIZE)
    def get_schedule(prompt: str, steps: int, use_scheduling: bool, seed: int | None) -> list[list[int | str | PromptParsingFailed]]:
        """Generate schedules for a single prompt with caching."""
        if seed is not None:
            random.seed(seed)
            ...
            if len(subprompts) > 1:
                ...
                for subprompt in subprompts:
                    ...
                    try:
                        ...
                        suggestion = "Ensure balanced brackets/parentheses and valid syntax (e.g., '[a:b:0.5]', 'a::b!')."
                        logging.warning(
                            f"Returning subprompt '{subprompt}' as-is due to parsing error. {reason}\nSuggestion: {suggestion}",
                            extra={'error_code': error_code}
                        )
                        if strict:
                            raise ValueError(f"Parsing failed for subprompt '{subprompt}': {reason}")
                        result.append([steps, PromptParsingFailed(subprompt, reason, error_code)])
                    except lark.exceptions.LarkError as e:
                        error_code = 'PRS002'
                        reason = f"General parsing error: {str(e)}"
                        suggestion = "Verify prompt syntax (e.g., '[a|b]', 'a AND b', 'a::b!')."
                        logging.warning(
                            f"Returning subprompt '{subprompt}' as-is due to parsing error. {reason}\nSuggestion: {suggestion}",
                            extra={'error_code': error_code}
                        )
                        ...
                        reason = str(e)
                        suggestion = {
                            "unbalanced": "Ensure all brackets and parentheses are properly closed (e.g., '[a:b:0.5]' or '(a:1.2)').",
                            "invalid weight": "Use valid numeric weights (e.g., '1.0' or '0.5').",
                            "unterminated sequence": "Terminate sequences with '!' or ';' (e.g., 'a::b!').",
                            "empty alternate": "Avoid empty alternate groups like '[||]'."
                        }.get(reason.lower().split(':')[0], "Check prompt syntax for errors.")
                        logging.warning(
                            f"Returning subprompt '{subprompt}' as-is due to validation error. {reason}\nSuggestion: {suggestion}",
                            extra={'error_code': error_code}
                        )
                        if strict:
                            raise ValueError(f"Validation failed for subprompt '{subprompt}': {reason}")
                        result.append([steps, PromptParsingFailed(subprompt, reason, error_code)])
                seen = set()
                final_result = []
        ...
        try:
            ...
            if strict:
                raise ValueError(f"Parsing failed for prompt '{prompt}': {reason}")
            ...
            logging.warning(
                f"Returning prompt '{prompt}' as-is due to validation error. {reason}\nSuggestion: {suggestion}",
                extra={'error_code': error_code}
            )
            if strict:
                raise ValueError(f"Validation failed for prompt '{prompt}': {reason}")
            return [[steps, PromptParsingFailed(prompt.strip(), reason, error_code)]]
            ...
            if strict:
                raise ValueError(f"Unexpected error parsing prompt '{prompt}': {reason}")
            return [[steps, PromptParsingFailed(prompt.strip(), reason, error_code)]]

    return [get_schedule(prompt, steps, use_scheduling, seed) for prompt in set(prompts)]
