...
def _concat_prefix_text_suffix(prefix: str, text: str, suffix: str) -> str:
    ...
    def _need_space(a: str, b: str) -> bool:
        ...
        # Unicode-безопасно: добавляем пробел только между ASCII alnum
        try:
            is_last_alnum = last_char.isascii() and last_char.isalnum()
            is_first_alnum = first_char.isascii() and first_char.isalnum()
            return is_last_alnum and is_first_alnum
        except:
            return False
...
class CollectSteps(lark.Visitor):
    ...
    def visit_alternate(self, tree):
        ...
        # ИСПРАВЛЕНИЕ: Удаляем последовательные пустые значения
        filtered_vals = []
        prev_empty = False
        for v in vals:
            is_empty = not v.strip()
            if not (is_empty and prev_empty):
                filtered_vals.append(v)
            prev_empty = is_empty

        vals = filtered_vals if filtered_vals else [""]
...
def _apply_and(text: str) -> str:
    # Примечание: & является внутренним представлением AND
    # после нормализации в _get_schedule_impl
    ...
    # 3) Гарантируем одиночный пробел вокруг слова-оператора AND или символа &
    # ИСПРАВЛЕНИЕ: Обрабатываем и слово, и символ
    text = re.sub(r'(?i)\s*(?<![\w_])and(?![\w_])\s*', ' and ', text)
    text = re.sub(r'\s*&\s*', ' and ', text)  # Теперь конвертируем & в ' and '
...
def _get_schedule_impl(prompt: str, steps: int, use_scheduling: bool, seed: int | None, use_visitor: bool):
    ...
    # ИСПРАВЛЕНИЕ: Единая нормализация AND → & (только здесь)
    prompt = _re.sub(r'(?i)\bAND\b', ATTENTION_AND_OPERATOR, prompt)
...
def _parse_prompt_attention_impl(text):
    ...
    for m in re_attention.finditer(text):
        ...
        if text_match.startswith('\\'):
            ...
            # ИСПРАВЛЕНИЕ: Более строгая логика
            pending_colon = True
            pending_colon_had_space = False
            pending_colon_position = len(res)  # Запоминаем позицию
            ...
            if pending_colon and not (round_brackets or square_brackets):
                # ИСПРАВЛЕНИЕ: Более строгая проверка числа
                ...
                if mnum:
                    ...
                    if j >= 0:
                        try:
                            ...
                            pending_colon = False
                            if not chunk.strip():  # Если ничего не осталось, не добавляем
                                continue
                        except ValueError:
                            pass
                
                # ИСПРАВЛЕНИЕ: Если это НЕ вес, но двоеточие уже было добавлено
                if pending_colon and pending_colon_position < len(res):
                    # Возвращаем двоеточие к предыдущему токену
                    j = last_token_index
                    while j >= 0 and res[j][0] in (':', 'BREAK'):
                        j -= 1
                    if j >= 0:
                        res[j][0] = res[j][0] + ":" + (" " if pending_colon_had_space else "")
                        # Удаляем отдельное двоеточие если оно было добавлено
                        if not SUPPRESS_STANDALONE_COLON and pending_colon_position < len(res):
                            if res[pending_colon_position][0] == ':':
                                ...
                                last_token_index = max(0, last_token_index - 1)
                    pending_colon = False
