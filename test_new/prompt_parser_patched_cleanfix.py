...
class ScheduleTransformer(lark.Transformer):
    ...
    def start(self, args):
        # Keep start minimal; rely on specific node handlers
    ...
    def nested_sequence(self, args):
        # Only treat '!', ';', '~' as explicit terminators
    ...
    def visit_emphasized(self, tree):
        ...
        if len(children) == 2:
            ...
            if isinstance(children[1], _l.Token) and getattr(children[1], "type", "") == "NUMBER":
                ...
                wt_str = resolve_tree(children[1], keep_spacing=True)
                try:
                    weight = float(wt_str)
                except Exception:
                    weight = 1.1
        ...
        return [[self.steps, self.prefix + f"({prompt_text}:{weight})" + self.suffix]]
        ...
        try:
            if len(parts) >= 2 and _re.fullmatch(r'[-+]?(?:\d+(?:\.\d+)?|\.\d+)(?:[eE][+-]?\d+)?', parts[-1]):
                ...
                if prompts:
                    ...
                    schedules = []
                    if int(round(step_size)) > 1 and (pre or post):
