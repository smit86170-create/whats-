...
def _concat_prefix_text_suffix(prefix: str, text: str, suffix: str) -> str:
    ...
    def _need_space(a: str, b: str) -> bool:
        ...
        try:
            # ИСПРАВЛЕНИЕ: Поддержка Unicode
            # Добавляем пробел между буквенно-цифровыми символами (любой язык)
            is_last_alnum = last_char.isalnum()
            is_first_alnum = first_char.isalnum()
...
def _apply_and(text: str) -> str:
    ...
    text = re.sub(r'(?i)\s*(?<![\w_])and(?![\w_])\s*', ' & ', text)
...
def get_schedule(prompt: str, steps: int, use_scheduling: bool, seed: int | None, use_visitor: bool = True):
    """
    ИСПРАВЛЕНИЕ: Документируем поведение с seed
    
    Args:
        seed: Если None, результаты с рандомизацией НЕ кэшируются и могут различаться
              между вызовами. Для воспроизводимости используйте конкретное число.
    """
    if seed is not None:
        return _get_schedule_cached(prompt, steps, use_scheduling, seed, use_visitor)
    else:
        # Для None генерируем временный seed для стабильности внутри одного вызова
        temp_seed = hash(prompt) % (2**31)
