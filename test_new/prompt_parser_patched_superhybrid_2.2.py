...
def _split_top_level_colon_all(s: str, keep_empty: bool) -> list[str]:
    """Разбить строку по ':' только на верхнем уровне (игнорируя ':' внутри круглых скобок и экранированные символы)."""
    parts: list[str] = []
    buf: list[str] = []
    depth = 0
    i = 0
    while i < len(s):
        ch = s[i]
        if ch == '\\':  # экранированный символ — добавляем оба и идём дальше
            if i + 1 < len(s):
...
def _build_bracket_after_schedules(
    ...
    Гибридная семантика:
      ...
      • если ЕСТЬ префикс/суффикса → «после N»: до N — пролог (pre+post), далее хвост.
    """
    ...
    if not prompts:
        # Нет промптов — весь прогон прологом
...
class CollectSteps(lark.Visitor):
        ...
        if m_rng and '|' not in full and '(' not in full and ')' not in full:
            ...
            def _to_steps_local(txt: str) -> int:
                ...
                try:
                    ...
                    return 1
            prompts = [p.strip() for p in _split_top_level_colon_keep_empty(inner)]
            ...
            return [[e, _apply_and(_collapse_spaces(t))] for e, t in schedules] or [[self.steps, _apply_and(_collapse_spaces(inner.strip()))]]
    ...
    def visit_start(self, tree):
        ...
        if m_ba and '|' not in full and full.count('[') == 1 and full.count(']') == 1:
...
def get_schedule(prompt: str, steps: int, use_scheduling: bool, seed: int | None, use_visitor: bool = True):
    ...
    if m_inner and prompt.count('[') == 1 and prompt.count(']') == 1:
        ...
        if '|' not in inner:  # не конфликтуем с альтернациями
            parts = _split_top_level_colon_keep_empty(inner)
    ...
    if m_with_pre and prompt.count('[') == 1 and prompt.count(']') == 1:
        ...
        try:
            ...
            inner_prompts = [p.strip() for p in _split_top_level_colon_keep_empty(inner)]
