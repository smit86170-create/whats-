...
top_level_sequence3: prompt ":::" sequence (WHITESPACE* "," WHITESPACE* sequence)* "!!!" (WHITESPACE* "," WHITESPACE* plain)?
...
class ScheduleTransformer(lark.Transformer):
    ...
    def start(self, args):
        ...
        # owner::a::b!!, extra -> owner -> owner: a, owner: b, extra
...
def get_schedule(prompt: str, steps: int, use_scheduling: bool, seed: int | None, use_visitor: bool = True):
    ...
    if m_toplevel:
        ...
        descriptors = [x.strip(' ,~!;') for x in rest.split('::') if x.strip(' ,~!;')]
        seqs = [f"{owner.strip()}: {d}" for d in descriptors]
        out = f"{owner.strip()} -> {', '.join(seqs)}"
        if trailing:
            out += f", {trailing.strip()}"
