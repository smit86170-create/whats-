...
class CollectSteps(lark.Visitor):
    ...
    def visit_scheduled(self, tree):
        ...
        # 1) Извлекаем промпты, исключая служебные узлы
        prompts = [
        ...
        # 2) Находим число (weight) и спец-узлы
        ...
        # Вспомогалка: число/проценты -> шаги
        def _to_steps(node_text: str) -> tuple[bool, float]:
            """Возвращает (is_percent, value). node_text — сырой текст, например '20' или '25%'."""
            s = node_text.strip()
            if s.endswith("%"):
                try:
        ...
        # 4) Считаем интервалы шагов
        step_intervals = []
        if step_range_list:
            # Обработка явных диапазонов: NUMBER-NUMBER или NUMBER%-NUMBER%
            for sr in step_range_list.children:
                ...
                start_node, end_node = sr.children

                # Берём исходный текст (универсально для Token/Tree)
                start_txt = resolve_tree(start_node, keep_spacing=False)
                end_txt = resolve_tree(end_node, keep_spacing=False)

                s_is_pct, s_val = _to_steps(start_txt)
                e_is_pct, e_val = _to_steps(end_txt)

                # Переводим в абсолютные шаги
                if s_is_pct:
                    start_step = int(round(s_val / 100.0 * self.steps))
                else:
                    start_step = int(round(s_val))

                if e_is_pct:
                    end_step = int(round(e_val / 100.0 * self.steps))
                else:
                    end_step = int(round(e_val))

                # Клампы и порядок
                start_step = _clamp_step(start_step)
                end_step = _clamp_step(end_step)
                if start_step < end_step:
                    step_intervals.append((start_step, end_step))
        else:
            # Равномерное деление по весу
            num_prompts = len(prompts)

            # Определяем "границу" по весу (как в A1111-логике)
            if weight <= 1.0:
                boundary = int(round(weight * self.steps))
            else:
                boundary = int(round(weight))
            boundary = _clamp_step(boundary)

            # Если boundary слишком мал, всё равно делим равномерно (минимум по 1 шагу)
            ...
            for i in range(num_prompts):
                start = int(round(i * step_size)) + 1  # шаги начинаются с 1
                end = int(round((i + 1) * step_size))
                start = _clamp_step(start)
                end = _clamp_step(end)
                if start < end:
        ...
        # 6) Формируем расписания (рекурсивно разворачиваем, если нужно)
        schedules = []
        for i, (start, end) in enumerate(step_intervals):
            # end — «граница» для следующего состояния
            end = min(end, self.steps)
            if start < end:
                p = prompts[i]
                if isinstance(p, lark.Tree):
                    child_schedules = self.visit(p)  # ожидается [[steps, text], ...]
                else:
                    # Текстовая нода
                ...
                for sched in child_schedules:
                    # Берём их текст и подставляем нашу границу end
                    schedules.append([end, self.prefix + sched[1] + self.suffix])

        # На случай пустоты — вернём сам prompt
        if not schedules:
