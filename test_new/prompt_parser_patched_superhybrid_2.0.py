...
RE_BRACKET_AFTER = re.compile(
    r'(?s)^(.*)\[(.*?)\]\s*:\s*([-+]?(?:\d+(?:\.\d+)?|\.\d+)(?:[eE][+-]?\d+)?)\s*(?:(?P<rev>r|reverse)\b)?\s*(?P<post>.*)$'
)
# Явные диапазоны: "[a:b]:10 1-4,5-7 [reverse]"
RE_BRACKET_AFTER_WITH_RANGES = re.compile(r'(?s)^\s*\[(?P<inner>.*?)\]\s*:\s*(?P<num>[-+]?(?:\d+(?:\.\d+)?|\.\d+)(?:[eE][+-]?\d+)?)\s+'
...
def _build_bracket_after_schedules(
    ...
    if not (pre.strip() or post.strip()):
        # «чистый» вид — интервалы внутри [1..N]
    ...
    # Пролог до N (добавляем, только если есть хвост)
    if tail > 0:
        schedules.append([boundary_end, _concat_prefix_text_suffix(pre, "", post)])

    if not prompts:
        # Нет промптов — весь прогоn прологом
        schedules.append([steps, _concat_prefix_text_suffix(pre, "", post)])
        return schedules

    if tail <= 0:
        # Хвоста нет — на последнем шаге используем последний вариант
        schedules.append([steps, _concat_prefix_text_suffix(pre, prompts[-1], post)])
    else:
        # Равномерно размазываем варианты по хвосту [N+1..steps]
        chunk = tail / max(1, len(prompts))
        for i, p in enumerate(prompts, start=1):
            end = boundary_end + int(round(i * chunk))
            end = max(boundary_end + 1, min(end, steps))
...
class ScheduleTransformer(lark.Transformer):
    ...
    def alternate(self, args):
        ...
        for arg in args:
            ...
            if tok in ("|", ",", "[", "]", "!"):
                if last_was_sep:
                    vals.append("")  # пустая опция между двумя сепараторами
                last_was_sep = True
                continue
            vals.append(s)
            last_was_sep = False
    ...
    def emphasized(self, args):
        ...
        # Соберём сырой текст аргументов
        raw_parts: list[str] = []
        for a in args:
        ...
        if len(parts) >= 2:
            # Стараемся вытащить ЧИСЛО "как есть" из второго куска (включая случаи "(2)")
            mnum = re.search(r"[-+]?(?:\d+(?:\.\d+)?|\.\d+)(?:[eE][+-]?\d+)?", parts[1])
            if mnum:
                num_txt = mnum.group(0)
                # Если целое — форматируем как 2.0, иначе оставляем исходную точность (1.25)
...
class CollectSteps(lark.Visitor):
    ...
    def visit_prompt(self, tree):
        ...
        if m_rng and '|' not in full and '(' not in full and ')' not in full:
            ...
            def _to_steps_local(txt: str) -> int:
                ...
                try:
                    ...
                    return 1
            prompts = [p.strip() for p in inner.split(':') if p.strip() or p == ""]
            ...
            return [[e, _apply_and(t)] for e, t in schedules] or [[self.steps, _apply_and(_collapse_spaces(inner.strip()))]]

        # (B) «Чистый» "[...]:N [reverse]" без диапазонов
    ...
    def visit_start(self, tree):
        ...
        if m_ba and '|' not in full and '(' not in full and ')' not in full:
            ...
            # Если regex "проглотил" reverse в post — аккуратно выделим его вручную
            if not rev_flag and post and re.match(r'^\s*(?:r|reverse)\b', post):
                rev_flag = 'reverse'
                post = re.sub(r'^\s*(?:r|reverse)\b\s*', '', post)
        ...
        # 2) [a:b:...:N] (без '|'), не трогаем reverse/диапазоны — они пойдут в visit_scheduled
    ...
    def _default_visit(self, tree):
        ...
        def _affix_text(seq):
            parts = []
            for c in seq:
                # не тащим в префикс/суффикс служебные узлы расписаний
                if isinstance(c, lark.Tree) and getattr(c, "data", None) in ("reverse_flag", "step_range_list"):
                    continue
                parts.append(resolve_tree(c, keep_spacing=True))
            return _norm_join_keep_edges("".join(parts))
        for i, child in enumerate(tree.children):
            # оставляем пробелы, чтобы выравнять поведение с get_schedule
            pre = _affix_text(c for j, c in enumerate(tree.children) if j < i)
            post = _affix_text(c for j, c in enumerate(tree.children) if j > i)

            if has_tree_child and isinstance(child, lark.Token):
                # если есть структурные узлы — сырые токены не посещаем (во избежание дублирования),
                # но префикс/суффикс мы уже собрали с их пробелами
                continue
    ...
    def visit_scheduled(self, tree):
        ...
        import lark as _l
        # Берём ТОЛЬКО поддеревья prompt (исключаем любые токены, step_range_list и reverse_flag),
        # иначе в prompts пролетают служебные символы и собирается мусор вроде "b 2-4,6-8".
        prompts = []
        ...
        if step_range_list:
            for sr in step_range_list.children:
                ...
                # принимаем оба варианта: проценты и абсолют
            ...
            # БЕЗ явных диапазонов — это "[...]: N" ПОСЛЕ границы:
            boundary = _clamp_step(int(round(weight * self.steps)) if weight <= 1.0 else int(round(weight)))
            prompt_texts = [resolve_tree(p, keep_spacing=True) for p in prompts]
            # Реверс — только когда НЕТ префикса/суффикса (т.е. «чистая» форма)
        ...
        if step_intervals:
            # Пролог:
            #  - ЧИСЛО: всегда (даже если старт == 1 → пролог на шаге 1);
            #  - ПРОЦЕНТ: только если старт > 1 (для паритета с get_schedule).
            pre_end = max(1, step_intervals[0][0] - 1)
            if first_is_percent:
                if step_intervals[0][0] > 1:
                    schedules.append([pre_end, _concat_prefix_text_suffix(self.prefix, "", self.suffix)])
            else:
                schedules.append([max(1, pre_end), _concat_prefix_text_suffix(self.prefix, "", self.suffix)])
        ...
        for i, (start, end) in enumerate(step_intervals[:len(prompts)]):
            ...
            if start < end:
                ...
                if isinstance(p, lark.Tree):
                    ...
                    child_texts = [s[1] for s in child_schedules] or [resolve_tree(p, keep_spacing=True)]
                else:
    ...
    def visit_alternate2(self, tree):
        options = [
            ...
            if resolve_tree(c, keep_spacing=True).strip() and resolve_tree(c, keep_spacing=True).strip() not in ("|", ",")
        ]
        suffix = options[0].split("_", 1)[1] if options and "_" in options[0] else ""
        combined = []
        for opt in options:
...
def get_schedule(prompt: str, steps: int, use_scheduling: bool, seed: int | None, use_visitor: bool = True):
    ...
    # Внутри скобок ...:N — учитываем переносы строк (DOTALL)
    ...
    if m_ranges:
        ...
        return [[e, _apply_and(_collapse_spaces(t))] for e, t in schedules] or [[steps, _apply_and(_collapse_spaces(inner.strip()))]]

    # "[...]:N" с префиксом/суффиксом — общий случай
    m_with_pre = _re.match(
        r'(?s)^(.*)\[(.*?)\]\s*:\s*([-+]?(?:\d+(?:\.\d+)?|\.\d+)(?:[eE][+-]?\d+)?)\s*(?:(?:r|reverse)\b)?\s*(.*)$',
        prompt
    )
    if m_with_pre:
        ...
        try:
            ...
            inner_prompts = [p.strip() for p in inner.split(':') if p.strip() or p == ""]

            # Вытащим reverse, если он в "post"
            rev_here = False
            m_rev = _re.match(r'^\s*(?:r|reverse)\b(.*)$', post or "")
            if m_rev:
                rev_here = True
                post = m_rev.group(1)
...
def _dedup_schedules(schedules: list[list[int, str]], joiner: str = ", ") -> list[list[int, str]]:
    ...
    return [[k, v] for k, v in sorted(last_by_step.items(),
                                      key=lambda kv: int(kv[0]) if isinstance(kv[0], (int,)) else kv[0])]
...
def parse_prompt_attention(text):
        ...
        if text_match.startswith('\\'):
            ...
            if pending_colon and not (round_brackets or square_brackets):
                ...
                if mnum:
                    ...
                    # это НЕ вес. Вернём двоеточие в текст.
                    j = last_token_index
                    while j >= 0 and res[j][0] in (':', 'BREAK'):
                        j -= 1
                    if j >= 0:
    ...
    rx_fix = re.compile(r'(\b[^\s:()\[\]{}]+)\s*:\s*([-+]?(?:\d+(?:\.\d+)?|\.\d+)(?:[eE][+-]?\d+)?)')
    used = {}
    ...
    def _smart_concat(a: str, b: str):
        ...
        ar = a.rstrip()
        # После знаков пунктуации (запятая/точка/точка с запятой/двоеточие) — пробел не обязателен,
        # но после закрывающих скобок оставляем то, что было в тексте.
        if ar and ar[-1] in ",.;:":
            a = ar
            b = b.lstrip()
            return a + b
