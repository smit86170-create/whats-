...
class CollectSteps(lark.Visitor):
        ...
        if step_range_list:
            ...
            for start, end in ranges:
                ...
                if start < end:
                    for prompt in prompts:
                        child_schedules = self.visit(prompt)
            ...
            for i, prompt in enumerate(prompts):
                ...
                if next_boundary > current_step:
                    child_schedules = self.visit(prompt)
                    for sched in child_schedules:
                        schedules.append([next_boundary, self.prefix + sched[1] + self.suffix])
    ...
    def visit_grouped(self, tree):
        ...
        max_combinations = min(self.MAX_COMBINATIONS, total_combinations)
        if total_combinations > max_combinations:
            all_options = [
                self.rng.sample(opt, max(1, max_combinations // len(all_options)))
                for opt in all_options
            ]
    ...
    def visit_nested_sequence(self, tree):
        elements = [resolve_tree(c, keep_spacing=True).strip(" ,~!;") for c in tree.children if resolve_tree(c, keep_spacing=True).strip(" ,~!;")]
        if any("~" in str(c) for c in tree.children):
