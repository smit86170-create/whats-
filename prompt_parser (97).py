...
def validate_prompt(prompt: str) -> bool:
    ...
    if re.search(r'\[\s*\|\s*\]', prompt) or re.search(r'\[\s*(\|\s*)+]', prompt):
        raise ValueError(f"Empty or invalid alternate group found in prompt: {prompt}")
    ...
    def get_schedule(prompt: str, steps: int, use_scheduling: bool, seed: int | None) -> list[list[int | str | PromptParsingFailed]]:
        ...
        if " AND " in prompt:
            ...
            if len(subprompts) > 1:
                ...
                expected_count = len(subprompts) + sum(1 for sp in subprompts if '[:' in sp and ':]' in sp)
                if len(final_result) > expected_count:
                    if strict:
                        raise ValueError(f"Invalid schedule generated for prompt '{prompt}': too many schedules {final_result}")
                    final_result = [s for s in final_result if not (isinstance(s[1], str) and ' ' in s[1] and any(s[1].startswith(p) for p in subprompts))]
